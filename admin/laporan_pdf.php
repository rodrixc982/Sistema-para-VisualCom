...
// mencetak string 
$pdf->Cell(280,7,'VISUALCOM',0,1,'C');
$pdf->SetFont('Arial','B',12);
$pdf->Cell(280,7,'ESTADOS CAJA',0,1,'C');

// Memberikan space kebawah agar tidak terlalu rapat
$pdf->Cell(10,7,'',0,1);
$pdf->SetFont('Arial','B',10);

$pdf->Cell(35,6,'PARTIR DE LA FECHA',0,0);
$pdf->Cell(5,6,':',0,0);
$pdf->Cell(35,6, date('d-m-Y', strtotime($tgl_dari)) ,0,0);
$pdf->Cell(10,7,'',0,1);
$pdf->Cell(35,6,'HASTA LA FECHA',0,0);
$pdf->Cell(5,6,':',0,0);
$pdf->Cell(35,6, date('d-m-Y', strtotime($tgl_sampai)) ,0,0);
$pdf->Cell(10,7,'',0,1);
$pdf->Cell(35,6,'CATEGORIA',0,0);
$pdf->Cell(5,6,':',0,0);
$kategori = $_GET['kategori'];
if($kategori == "semua"){
  $kkk = "TODAS LAS CATEGORIAS";
}else{
...
$pdf->Cell(35,14,'FECHA',1,0,'C');
$pdf->Cell(45,14, 'CATEGORIA' ,1,0,'C');
$pdf->Cell(105,14,'INFORMACION',1,0,'C');

$pdf->Cell(82,7,'TIPO',1,0,'C');
$pdf->Cell(1,7,'',0,1);
$pdf->Cell(195,7,'',0,0);
$pdf->Cell(41,7,'INGRESOS',1,0,'C');
$pdf->Cell(41,7,'GASTOS',1,1,'C');
...
while($d = mysqli_fetch_array($data)){
  ...
  if($d['transaksi_jenis'] == "Pemasukan"){
    $pem = "S/. ".number_format($d['transaksi_nominal'])." ";
  }else{
    $pem = "-";
  }

  if($d['transaksi_jenis'] == "Pengeluaran"){
    $peng = "S/. ".number_format($d['transaksi_nominal'])." ";
  }else{
    $peng = "-";
  }
...
$pdf->Cell(41,7,"S/. ".number_format($total_pemasukan)." ",1,0,'C');
$pdf->Cell(41,7,"S/. ".number_format($total_pengeluaran)." ",1,1,'C');

$pdf->Cell(10,0,'',0,1);

$pdf->Cell(195,7, "SALDO" ,1,0,'R');
$pdf->Cell(82,7,"S/. ".number_format($total_pemasukan - $total_pengeluaran)." ",1,0,'C');
